...
for source in "${sources[@]}"; do
    origin="docs/contributing/tasks/$(basename "$source")"
    ...
    write_target "$OUT_ROOT/.claude/commands/$name.md" \
        "---
description: $description
---" "$origin" "$source"
    generated+=(".claude/commands/$name.md")

    # Cursor command: /<name>
    write_target "$OUT_ROOT/.cursor/commands/$name.md" \
        "" "$origin" "$source"
    generated+=(".cursor/commands/$name.md")

    # Windsurf workflow: /<name>
    write_target "$OUT_ROOT/.windsurf/workflows/$name.md" \
        "---
description: $description
---" "$origin" "$source"
    generated+=(".windsurf/workflows/$name.md")
...
shopt -s nullglob
for existing in "$ROOT"/.claude/commands/*.md "$ROOT"/.cursor/commands/*.md \
    "$ROOT"/.windsurf/workflows/*.md "$ROOT"/.github/prompts/*.prompt.md; do
